/**
 * Infil Cards Overlay — miniature backcover sous le support overlay, côté gauche.
 * - Hover sur la miniature : affiche le panneau de cartes infil
...
export class InfilOverlay {
  ...
  /** UUIDs des cartes infil actives. */
  static get cardIds() {
  ...
  static async render() {
    ...
    el.innerHTML = `
      <div class="haywire-support-thumb" title="${i18n("HAYWIRE.Infil.Label")}">
        <img src="systems/haywire/assets/cards/backcovers/infil.webp" alt="Infil" />
  ...
  static #getOrCreate() {
    ...
    if (!this.#panelEl) {
      ...
      this.#panelEl.id = "haywire-infil-panel";
      ...
      panel.innerHTML = `
        <div class="haywire-support-panel-inner">
          <div class="haywire-support-panel-header">
            <i class="fas fa-user-secret"></i> ${i18n("HAYWIRE.Infil.Label")}
          </div>
          <div class="haywire-support-empty">${i18n("HAYWIRE.Infil.Empty")}</div>
        </div>`;
      return;
    }
      ...
      .map((uuid, i) => {
        ...
        <div class="haywire-support-card" data-preview-img="${img}" data-preview-name="${name}">
          ...
          <button class="haywire-support-activate" data-uuid="${uuid}" data-name="${name}" data-img="${img}"
                  title="${i18n("HAYWIRE.Infil.Activate")}">
            <i class="fas fa-bullseye"></i> ${i18n("HAYWIRE.Infil.Activate")}
          </button>
        </div>`;
      })
      .join("");
    ...
    await ChatMessage.create({
      content: `<div class="haywire-card-chat">
        <div class="haywire-card-chat-header">
          <i class="fas fa-user-secret"></i> ${i18n("HAYWIRE.Infil.Activated")}
        </div>
